    ...
    >
      <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto px-1 py-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:snap-none sm:grid-cols-2 sm:overflow-visible lg:block lg:p-0">
        {technologies.map(({ name, area, icon: Icon, duration, featured, position }) => (
          <m.article
            className={`relative z-10 flex h-24 w-40 shrink-0 snap-start items-center gap-3 rounded-[18px] border border-border bg-surface/85 p-4 shadow-card backdrop-blur-xl transition-[box-shadow,border-color] duration-200 hover:border-primary/25 hover:shadow-[0_16px_40px_rgba(37,99,235,0.12)] sm:w-auto ${position} lg:absolute lg:w-44 ${featured ? 'lg:h-26 lg:w-48' : ''}`}
            key={name}
