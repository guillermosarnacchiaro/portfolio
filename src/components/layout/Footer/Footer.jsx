...
const links = [
  ...
  ['Descargar CV', '/cv/guillermo-sarnacchiaro-cv.pdf'],
]
    ...
    <footer className="border-t border-border py-10 text-sm text-subtle">
      <Container>
          ...
          <nav aria-label="Enlaces sociales">
            <ul className="flex flex-wrap gap-x-5 gap-y-3">
              {links.map(([label, href]) => (
                <li key={label}>
                  <a className="inline-flex items-center gap-1 transition-colors duration-200 hover:text-primary-readable" href={href} target={href.startsWith('http') ? '_blank' : undefined} rel={href.startsWith('http') ? 'noreferrer' : undefined} download={href.endsWith('.pdf') ? true : undefined} aria-label={href.startsWith('http') ? `${label}, abre en una pestaña nueva` : label}>
        ...
        <div className="mt-8 flex flex-col gap-2 border-t border-border pt-6 text-xs sm:flex-row sm:justify-between">
          <p>© {currentYear} Guillermo Sarnacchiaro</p>
